...
class Boot
{
    ...
    public function __construct($configurations = [])
    {
        ...
        $root_path = $is_in_plugin
            ? substr($root_dir, strpos($root_dir, "/plugins"))
            : substr($root_dir, strpos($root_dir, "/themes"));

        defined("WP_COMPONENTS_ASSETS") or
            define(
                "WP_COMPONENTS_ASSETS",
                content_url() . $root_path . "/public/",
            );
        defined("WP_COMPONENTS_PATH") or
            define("WP_COMPONENTS_PATH", $root_dir . "/src/");
        defined("WP_COMPONENTS_LANGUAGE") or
            define("WP_COMPONENTS_LANGUAGE", $this->configurations["language"]);
    ...
    private function hook(): void
    {
        ...
        add_action("enqueue_block_editor_assets", [$this, "enqueue_scripts_styles"]);
        ...
        if (class_exists("WooCommerce")) {
            ...
            add_filter("woocommerce_add_to_cart_fragments", function (
                ...
                $fragments["span.atom-cart-count"] =
                    '<span class="atom-cart-count">' .
    ...
    public function enqueue_scripts_styles()
    {
        ...
        $version_file = require_once(WP_COMPONENTS_PATH . "assets/version.php");
        ...
        if ($this->configurations["tinyslider"]) {
            wp_enqueue_style(
                "tinyslider-css",
                WP_COMPONENTS_ASSETS . "vendor/css/tinyslider.min.css",
            );
            wp_enqueue_script(
                "tinyslider-js",
                WP_COMPONENTS_ASSETS . "vendor/js/tinyslider.min.js",
                [],
                null,
                true,
            );
        ...
        if ($this->configurations["scrollreveal"]) {
            wp_enqueue_script(
                "scrollreveal-js",
                WP_COMPONENTS_ASSETS . "vendor/js/scrollreveal.min.js",
                [],
                null,
                true,
            );
        ...
        if ($this->configurations["css"]) {
            wp_enqueue_style(
                "wpc-css",
                WP_COMPONENTS_ASSETS . "wpc-styles.min.css",
                [],
                $version
            );
        }
        ...
        if ($this->configurations["animate"]) {
            wp_enqueue_style(
                "wpc-animate-css",
                WP_COMPONENTS_ASSETS . "vendor/css/animate.min.css",
            );
        }
        ...
        if ($this->configurations["hover"]) {
            wp_enqueue_style(
                "wpc-hover-css",
                WP_COMPONENTS_ASSETS . "vendor/css/hover.min.css",
            );
        }
        ...
        if ($this->configurations["js"]) {
            wp_enqueue_script(
                "wpc-scripts-js",
                WP_COMPONENTS_ASSETS . "wpc-scripts.min.js",
                [],
                $version,
                true,
            );
            ...
            // Localize our script
            wp_localize_script("wpc-scripts-js", "wpc", [
